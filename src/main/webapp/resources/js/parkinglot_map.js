...
for(var i = 0; i < publicDatas.length; i++){
   ...
   publicContents[i] += '        <img src="../resources/img/parkinglot_blank3.png" alt="" style="width: 90px; height: 60px;">';
   publicContents[i] += '      <div>';
   publicContents[i] += '      <center>';
   publicContents[i] += '        <span class="address">';
   publicContents[i] += '<i class="fa fa-map-marker" style="color: #38D3FF;font-size: 15px;"></i>&nbsp;';
   publicContents[i] += publicDatas[i][3];  //주차장 주소
   publicContents[i] += '        </span>';
   publicContents[i] += '        <span class="address" style="margin-top: 25px;">';
   publicContents[i] += '최초 ' + publicDatas[i][6] + '분 : ' + publicDatas[i][5] +'원';  //기본요금정보
   publicContents[i] += '<br>추가 ' + publicDatas[i][8] + '분마다 ' + publicDatas[i][7] +'원';  //추가요금정보
   publicContents[i] += '        </span>';
   publicContents[i] += '      </center>';
   publicContents[i] += '      </div>';
   publicContents[i] += '    </div>';
...
for(var i = 0; i < shareDatas.length; i++){
   ...
   shareContents[i] += '" style="width: 90px; height: 60px;" alt="">';

   shareContents[i] += '      <div>';
   shareContents[i] += '      <center>';
   shareContents[i] += '        <span class="address"">';
   shareContents[i] += '<i class="fa fa-map-marker" style="color: #38D3FF;font-size: 15px;"></i>&nbsp;';
   shareContents[i] += shareDatas[i][3];  //주차장 주소
   shareContents[i] += '        </span>';
   shareContents[i] += '        <span class="address" style="font-size: 15px; margin-top: 25px;">';
   shareContents[i] += '<i class="fa fa-usd" style="color: #38D3FF;font-size: 15px;"></i>&nbsp;';
   shareContents[i] += shareDatas[i][5];  //기본요금정보
   shareContents[i] += '원 / 1시간 ';
   shareContents[i] += '        </span>';
   shareContents[i] += '      </center>';
   shareContents[i] += '      </div>';
   shareContents[i] += '    </div>';
